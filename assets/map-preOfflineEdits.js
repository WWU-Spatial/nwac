...
//	}); 
	...
	// moved to mobileInit function in html head
...
function init() {
	...
	//  center map on all regions
//	map.centerAndZoom(initExtent.getCenter,8);
...
//	refreshUserInfo('benkane11@gmail.com','Ben','Kane');
	
	// Look for stored user info and use it for forms 
	if ( useLocalStorage ) {
	      var userInfoJSON = window.localStorage.getItem(storeUser);
	    } else {
	      var userInfoJSON = dojo.cookie(storeUser);
	    }
	    // Load user info 
	    // Fall back to a single bookmark if no cookie
	if ( userInfoJSON && userInfoJSON != 'null'/* && userInfoJSON.length > 2*/) {
	  var userInfo = dojo.fromJson(userInfoJSON);
...
//		console.log('no parsed.objects') /* just make the page */
	}
...
//    $('#infoDiv').remove();
	$('.esriSimpleSlider').css({visibility: "visible"});
...
//		console.log($(zNum[0].childNodes),dateList);
//		$(zNum[0].childNodes.children()).each(function(node){
//			console.log(node);
//			node?console.log(node.childNodes):console.log('undefined');
//		});
...
function addPageSwipeListeners(page){
	...
	// TODO -- figure out how to reset graphic from here and also make sure to do this only if coming from attributes page
//	hideGoToAttsDiv(); 
}
...
//	
	...
	///
	var fullPT = {'point': new esri.geometry.Point(-13521308.650948754, 5826008.784827091, SR), 'level': 6};
	var OlympicsPT = {'point': new esri.geometry.Point(-13769917.244339418, 6020431.108485553, SR), 'level': 9};
    var MtHoodPT = {'point': new esri.geometry.Point(-13554976.320801608, 5642220.692530684, SR), 'level': 9};
    var StevensPassPT = {'point': new esri.geometry.Point(-13480396.74151801, 6043270.487268172, SR), 'level': 10};
    var SnoqPassPT = {'point': new esri.geometry.Point(-13516933.64103834, 5987840.391056036, SR), 'level': 10};
    var WhitePassPT = {'point': new esri.geometry.Point(-13510818.67877556, 5855936.581646458, SR), 'level': 10};
    var WestStevensNorthPT = {'point': new esri.geometry.Point(-13502445.022642313, 6104318.377371862, SR), 'level': 8};
    var WestStevensToSnoqPT = {'point': new esri.geometry.Point(-13533631.462262811, 6002457.104711823, SR), 'level': 9};
    var WestSnoqToWhitePT = {'point': new esri.geometry.Point(-13562983.281124303, 5891164.79152866, SR), 'level': 9};
    var WestWhiteSouthPT = {'point': new esri.geometry.Point(-13585608.641496703, 5771527.591557064, SR), 'level': 9};
    var EastStevensNorthPT = {'point': new esri.geometry.Point(-13392681.582105009, 6101572.496190777, SR), 'level': 8};
    var EastStevensToSnoqPT = {'point': new esri.geometry.Point(-13440939.23492287, 5991457.104711815, SR), 'level': 9};                                       
    var EastSnoqToWhitePT = {'point': new esri.geometry.Point(-13496024.250017693, 5889177.419305798, SR), 'level': 9};
    var EastWhiteSouthPT = {'point': new esri.geometry.Point(-13481654.28302891, 5790008.23682748, SR), 'level': 9};
	...
	var pointList = [fullPT, OlympicsPT, MtHoodPT, StevensPassPT, SnoqPassPT, WhitePassPT, WestStevensNorthPT, WestSnoqToWhitePT,
    WestStevensToSnoqPT, WestWhiteSouthPT, EastStevensNorthPT, EastStevensToSnoqPT, EastSnoqToWhitePT, EastWhiteSouthPT];

//    map.setExtent(extentList[value]);
	
	////
//	$.each(extentList, function(i){
//		console.log(extentList[i].getCenter());
//	})
	
//	map.centerAt(pointList[value]);
	var center = extentList[value].getCenter();
//	console.log(center)

	map.centerAndZoom(center,pointList[value].level);
//	map.centerAndZoom(pointList[value].point,pointList[value].level);
	////
...
function getRegion(evt) { 
	...
	queryTask.execute(query, function updateRegion(result){
		...
		if(!parsed.features[0]){
			...
			zone = regionNum;
//			console.log(parsed.features[0], map.extent, region, map.getLevel(), evt.mapPoint );//, regionNum,zone,regions[zone]);
		}
	});
}
