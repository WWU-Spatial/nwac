	...
	$.extend( $.mobile.datebox.prototype._build, {
		'calbox': function () {
			...
			if ( o.calUsePickers === true ) {
				...
				cal.picker1.live('change', function () { w.theDate.setMonth($(this).val()); w.refresh(); });
				cal.picker2.live('change', function () { w.theDate.setFullYear($(this).val()); w.refresh(); });
			...
			w.d.intHTML.live(o.clickEventAlt+' vmouseover vmouseout', 'div.'+uid+'griddate', function(e) {
			...
			w.d.intHTML
				.live('swipeleft', function() { if ( w.calNext ) { w._offset('m', 1); } })
				.live('swiperight', function() { if ( w.calPrev ) { w._offset('m', -1); } });
			
			if ( w.wheelExists) { // Mousewheel operations, if plugin is loaded
				w.d.intHTML.live('mousewheel', function(e,d) {
					e.preventDefault();
					if ( d > 0 && w.calNext ) { 
						w.theDate.set(2,1);
						w._offset('m', 1);
